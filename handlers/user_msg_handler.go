...
// ReplyText 发送文本消息到群
func (u *UserMessageHandler) ReplyText() error {
	logger.Info(fmt.Sprintf("Received User %v Text Msg : %v", u.sender.NickName, u.msg.Content))
	// 1.获取上下文，如果字符串为空不处理
	requestText := u.getRequestText()
	if requestText == "" {
	...
	if match == true {
		...
		reply, err := gpt.Completions(u.getRequestText())
		...
		// 2.设置上下文，回复用户
		u.service.SetUserSessionContext(requestText, reply)
		_, err = u.msg.ReplyText(buildUserReply(reply))
		if err != nil {
			return errors.New(fmt.Sprintf("response user error: %v ", err))
		}
